...
render_no_write() {
  helm template rbac-check "$CHART_DIR" \
    ...
    --set rbac.namespaces.targets='{observability,sops-secrets,argocd}' \
...
render_no_write_flux() {
  helm template rbac-check "$CHART_DIR" \
    ...
    --set rbac.namespaces.targets='{observability,sops-secrets,flux-system}' \
    --set prometheusStack.enabled=true \
    --set sops.enabled=true
}
...
# rather than inert. It must not receive write from this template even when
# explicitly listed in rbac.namespaces.targets (as it is in render_no_write
# above — otherwise this assertion is vacuously green).
...
# Same for Flux: flux-system is listed in render_no_write_flux's targets and
# must still be excluded from the write RoleBinding.
no_write_flux_namespaces="$(render_no_write_flux | rolebinding_namespaces | sort -u)"
if grep -qx 'flux-system' <<<"$no_write_flux_namespaces"; then
  echo "no-write Flux mode rendered s9s-write RoleBinding in the GitOps controller namespace (flux-system)" >&2
  exit 1
fi
